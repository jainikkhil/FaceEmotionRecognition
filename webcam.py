...
def start_analyzing_mimic():
    ...
    while True:
        ret, frame = cap.read()
        result_analyzer = DeepFace.analyze(frame, actions=['emotion'], enforce_detection=False)['emotion']['dominant']
...
def analyze_expressions(emotion_list):
    for emotion_dict in emotion_list:
        for key in emotion_dict:
            emotions[key] += emotion_dict[key]
